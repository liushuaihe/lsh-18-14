...
import {
  ...
  AlertTriangle, Clock, Activity, Server, Battery, Zap,
} from 'lucide-react';

export const TopBar: React.FC = () => {
  const { devices, sensors, globalFuseActive, alerts, powerMode, upsBattery } = useMonitorStore();
      ...
      <div className="flex-1 flex items-center justify-center gap-1.5 flex-wrap">
        ...
        {powerMode !== 'mains' && (
          <>
            <Divider />
            <StatChip
              icon={powerMode === 'ups' ? <Battery size={11} /> : <Zap size={11} />}
              label={powerMode === 'ups' ? 'UPS' : '断电'}
              value={powerMode === 'ups'
                ? `${upsBattery.healthPercent.toFixed(0)}%`
                : 'OFF'
              }
              sub={powerMode === 'ups'
                ? `${upsBattery.remainingMinutes < Infinity ? Math.ceil(upsBattery.remainingMinutes) + 'min' : '--'}`
                : undefined
              }
              color={powerMode === 'ups' ? 'amber' : 'red'}
              blink={powerMode === 'off' || (powerMode === 'ups' && upsBattery.healthPercent < 20)}
            />
          </>
        )}
      </div>
