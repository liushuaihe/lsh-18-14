...
import { MOCK_DEVICES, MOCK_SENSORS, MOCK_UPS_BATTERY } from '../data/mockData';
import {
  canDeviceStart,
  getDependencyOrder,
...
import {
  ...
  executePriorityShutdown,
  tickBatteryDrain,
} from '../engine/upsEngine';

interface UpsShutdownEntry {
  deviceId: string;
  deviceName: string;
  priority: number;
  timestamp: number;
...
interface MonitorStore {
  ...
  powerMode: PowerMode;
  shutdownTimeline: ShutdownStep[];
  shutdownLog: UpsShutdownEntry[];
  nextShutdownPriority: number;
  upsActive: boolean;
  simSpeed: number;
  ...
  simulateMainPowerFailure: () => void;
  restoreMainPower: () => void;
  tickUPS: (elapsedSeconds: number) => void;
  setUpsBattery: (battery: UPSBattery) => void;
...
export const useMonitorStore = create<MonitorStore>((set, get) => ({
  ...
  simulateMainPowerFailure: () => {
    ...
    newAlerts.unshift({
      ...
      sensorId: 'T-PWR-01',
      level: 'critical',
      message: '⚡ 主电源故障 — UPS电池已接管供电',
      acknowledged: false,
    });
    set({ alerts: newAlerts.slice(0, 100) });
  },
  ...
  restoreMainPower: () => {
    ...
    const battery: UPSBattery = {
      ...state.upsBattery,
      loadKW: getCurrentLoadKW(state.devices),
      charging: true,
    };
    ...
    newAlerts.unshift({
      ...
      sensorId: 'T-PWR-01',
      level: 'warning',
      message: '✅ 主电源已恢复 — UPS切换至充电模式',
      acknowledged: false,
    });
    set({ alerts: newAlerts.slice(0, 100) });
  },

  tickUPS: (elapsedSeconds: number) => {
    const state = get();
    if (state.powerMode !== 'ups' || !state.upsActive) return;
    ...
    if (changed) {
      const newLoad = getCurrentLoadKW(devices);
      battery = {
        ...battery,
        loadKW: newLoad,
        remainingMinutes: calculateRemainingMinutes({ ...battery, loadKW: newLoad }),
      };

      const newSensors: Record<string, TempSensor> = {};
      for (const [id, s] of Object.entries(state.sensors)) {
        const dev = devices[s.deviceId];
        if (dev && !isDeviceRunning(dev.status)) {
          newSensors[id] = { ...s, currentTemp: s.baseTemp, alertLevel: 'normal' };
        } else {
          newSensors[id] = s;
        }
      }
      ...
      if (battery.currentCapacityKWh <= 0) {
        const finalDevices: Record<string, Device> = {};
        for (const [id, dev] of Object.entries(devices)) {
          if (isDeviceRunning(dev.status)) {
            finalDevices[id] = { ...dev, status: DeviceStatus.OFF, lastStatusChange: Date.now() };
          } else {
            finalDevices[id] = dev;
          }
        }
        set({
          ...
          powerMode: 'off',
          upsActive: false,
          upsBattery: { ...battery, loadKW: 0, remainingMinutes: 0, healthPercent: 0 },
          shutdownTimeline: [],
      ...
      set({
        devices,
        sensors: recomputeAllThresholds(devices, newSensors),
        upsBattery: battery,
        shutdownTimeline: timeline,
  ...
  setUpsBattery: (battery: UPSBattery) => {
    set({ upsBattery: battery });
  },
